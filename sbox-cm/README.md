...
六个 op 全部 dispatch 给 root worker —— op 语义只有一份实现。
...
凭据**绝不跨越 privileged worker 信任边界**：

```text
不得进入：Python daemon / RPC / argv / env / stdout / stderr /
          journald / audit / journal / client.list / error detail
仅存在于：worker 进程树内存、匿名管道 / 私有 FD、live 服务端配置、root-only 账本中的不可逆 digest
```
...
bash tests/e3/test-m1-static.sh        # 单引擎 / 六 op / systemd 候选 / 无隐式激活
bash tests/e3/test-m1-worker.sh        # 六个 op、ledger、幂等、调和、degraded、回滚、锁失败、/proc 动态卫生
bash tests/e3/test-m1-rpc.sh           # schema + AST 不变量 + frame + peer(含 uid 0) + replay
...
client.rotate / client.export / client.get                        → 未纳入（须新一轮论证）
Web 凭据下载                                                       → 推迟（credential_delivery:"cli"）
